...
Для работы приложения требуется база данных PostgreSQL с названием social_messenger. (Подключение по умолчанию postgres postgres)
...
Реализованные основные задания: 
	...
	* Аутентификация (необходимый API, поддержка Spring Security, информация о сессии в JWT и передача их в HTTP хедерах, поддержка Refresh-токенов )
...
Описание и примеры REST-запросов:
	AuthController:
		POST    /auth/login - проводит аутентификацию пользователя и возвращает Access и Refresh токены
			URL example: localhost:8081/auth/login
			Payload: JwtRequest(input), JwtResponse(output)
			Response: 200 OK (успешная аутентификация и выдача токенов)
        ...
        POST    /auth/refresh - обновляет токены пользователя и возвращает их
            URL example: localhost:8081/auth/refresh
            Payload: String(input), JwtResponse(output)
            Response: 200 OK (токены успешно обновлены и выданы)
                  403 Forbidden (предоставлен неправильный или недействительный Refresh-токен)
=========================================================================================================================		
	ProfileController:
